package io.themade4.relictium.common.config;
 ...
 */
public class RelictiumConfig {
    private static final Logger LOGGER = LogManager.getLogger(Tags.MOD_NAME + "Config");
    ...
    ).map(RelictiumConfig::getMixinRuleName).collect(Collectors.toSet());

    private final Map<String, Option> options = new HashMap<>();

    private RelictiumConfig() {
     ...
     */
    public static RelictiumConfig load(File file) {
        if (!file.exists()) {
            try {
        ...
        RelictiumConfig config = new RelictiumConfig();
    ...
    private static void writeDefaultConfig(File file) throws IOException {
        ...
        try (Writer writer = new FileWriter(file)) {
            writer.write("# This is the configuration file for " + Tags.MOD_NAME + ".\n");
            writer.write("#\n");
