package io.themade4.relictium.mixin;

import io.themade4.relictium.RelictiumClientMod;
import io.themade4.relictium.Tags;
import io.themade4.relictium.common.config.Option;
import io.themade4.relictium.common.config.RelictiumConfig;
...
public class RelictiumMixinPlugin implements IMixinConfigPlugin {
    private static final String MIXIN_PACKAGE_ROOT = "io.themade4.relictium.mixin.";

    private final Logger logger = LogManager.getLogger(Tags.MOD_NAME);
    private RelictiumConfig config;

    @Override
    public void onLoad(String mixinPackage) {
        try {
            this.config = RelictiumConfig.load(new File(".").toPath().resolve("config").resolve(Tags.MOD_ID + "-mixins.properties").toFile());
        } catch (Exception e) {
            throw new RuntimeException("Could not load configuration file for " + Tags.MOD_NAME, e);
        }

        this.logger.info("Loaded configuration file for " + Tags.MOD_NAME + ": {} options available, {} override(s) found",
