package io.themade4.relictium.core.client.model.light;
        ...
        for (int bX = bMinX; bX < bMaxX; bX++) {
            ...
            for (int bY = bMinY; bY < bMaxY; bY++) {
                ...
                for (int bZ = bMinZ; bZ < bMaxZ; bZ++) {
                    ...
                    sl += weight * (entity.world.getLightFor(EnumSkyBlock.SKY, pos) / MAX_LIGHT_VAL);

                    if (calcBlockLight) {
                        bl += weight * (entity.world.getLightFor(EnumSkyBlock.BLOCK, pos) / MAX_LIGHT_VAL);
                    } else {
                        bl += weight;
                    }
                }
